...
public class HTTPClientRequestThread extends Thread {
	...
	public void run()
	{
		try 
		{
			...
			new HTTPResponse(filtered_page).send(socket, (request.getMethod().equals("GET")));
		...
		{
			try {
				new HTTPResponse(HTTPResponse.INTERNAL_SERVER_ERROR_500).send(socket, true);
			} catch (IOException e1) { }
	...
	private HTMLPage getPageFromRemote(URL url) throws RemoteConnectionException, HTMLParsingException
	{
		...
		catch(IOException e)
		{
			try {
				...
				try {
					new HTTPResponse(502).send(socket, true);
